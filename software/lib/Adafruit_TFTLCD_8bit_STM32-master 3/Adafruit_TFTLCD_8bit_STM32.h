...
#define WR_ACTIVE                 \
  {                               \
    cntrlRegs->BRR = TFT_WR_MASK; \
  }
#define WR_IDLE                    \
  {                                \
    cntrlRegs->BSRR = TFT_WR_MASK; \
  }
#define CD_COMMAND                \
  {                               \
    cntrlRegs->BRR = TFT_RS_MASK; \
  }
#define CD_DATA                    \
  {                                \
    cntrlRegs->BSRR = TFT_RS_MASK; \
  }
#define CS_ACTIVE                 \
  {                               \
    cntrlRegs->BRR = TFT_CS_MASK; \
  }
#define CS_IDLE                    \
  {                                \
    cntrlRegs->BSRR = TFT_CS_MASK; \
  }
#define CS_ACTIVE_CD_COMMAND                      \
  {                                               \
    cntrlRegs->BRR = (TFT_CS_MASK | TFT_RS_MASK); \
  }
#endif
...
#define write8(c)                                         \
  {                                                       \
    dataRegs->BSRR = (uint32_t)(0x00FF0000 + ((c)&0xFF)); \
    WR_STROBE;                                            \
  }
...
#define write8(c)                                                             \
  {                                                                           \
    dataRegs->BSRR = (uint32_t)(0xFF000000 + (((c)&0xFF) << TFT_DATA_SHIFT)); \
    WR_STROBE;                                                                \
  }
// inline void write8(uint8_t d) { dataRegs->BSRR = (uint32_t)(0x00FF0000 + d); WR_STROBE; };
#endif
